...
class Snake(QtWidgets.QWidget):
    ...
    def initUI(self):
        ...
        GPIO.add_event_detect(8, GPIO.RISING, callback=self.up)
        GPIO.add_event_detect(10, GPIO.RISING, callback=self.down)
        GPIO.add_event_detect(12, GPIO.RISING, callback=self.left)
        GPIO.add_event_detect(16, GPIO.RISING, callback=self.right)
        self.show()
    ...
    def up(self, e):
        if not self.isPaused and self.lastKeyPress != 'UP' and self.lastKeyPress != 'DOWN':
            self.direction("UP")
            self.lastKeyPress = 'UP'

    def down(self, e):
        if not self.isPaused and self.lastKeyPress != 'DOWN' and self.lastKeyPress != 'UP':
            self.direction("DOWN")
            self.lastKeyPress = 'DOWN'

    def left(self, e):
        if not self.isPaused and self.lastKeyPress != 'LEFT' and self.lastKeyPress != 'RIGHT':
            self.direction("LEFT")
            self.lastKeyPress = 'LEFT'

    def right(self, e):
        if not self.isPaused and self.lastKeyPress != 'RIGHT' and self.lastKeyPress != 'LEFT':
